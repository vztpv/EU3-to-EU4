...
namespace clau_parser {
	...
	public:
		...
		size_t GetUserTypeListCapacity() const { return userTypeList.capacity(); }
	...
	public:
		static int Merge(UserType* next, UserType* ut, UserType** ut_next, UserType** out = nullptr)
		{
			...
			//check!!
			while (ut->GetUserTypeListSize() >= 1
	...
	private:
		...
		static bool _LoadData(InFileReserver& reserver, UserType& global, const int lex_thr_num, const int parse_num) // first, strVec.empty() must be true!!
		{
			...
			for (long long i = 0; i < token_arr_len - 1; ++i) {
				if (Utility::GetType(token_arr[i]) == TYPE_LEFT) {
					count++;
				}
				else if (Utility::GetType(token_arr[i]) == TYPE_RIGHT) {
					count--;
				}
			}

			bool do_except_last = false;
			if (count == 0 && Utility::GetType(token_arr[token_arr_len - 1]) == TYPE_RIGHT) {
				do_except_last = true;
			}

			if (Utility::GetType(token_arr[token_arr_len - 1]) == TYPE_LEFT) {
				count++;
			}
			else if (Utility::GetType(token_arr[token_arr_len - 1]) == TYPE_RIGHT) {
				count--;
			}
		...
		static long long Count(UserType* x) {
			long long sum = x->GetItemListCapacity() + x->GetUserTypeListCapacity();
			for (int i = 0; i < x->GetUserTypeListSize(); ++i) {
				sum += Count(x->GetUserTypeList(i));
			}
			return sum;
		}
	...
	public:
		static bool LoadDataFromFile(const std::wstring& fileName, UserType& global, int lex_thr_num = 1, int parse_thr_num = 1, bool use_simd = false) /// global should be empty
		{
			if (lex_thr_num <= 0) {
			...
			//std::cout << "Count is " << Count(&global) << "\n";
		...
		static bool Load(UserType& global, const std::wstring& fileName, const int thr_num) {
		...
		static bool Save(const UserType& global, const std::wstring& fileName, const bool append = false) {
	...
	public:
		std::string GetKey() const {
			if (IsGroup()) {
				bool err = false;
				long long ut_idx = _stack.back()->GetUserTypeIndexFromIlistIndex(idx, err);
				if (err) { return "#1"; }
				return _stack.back()->GetUserTypeList(ut_idx)->GetName();
			}
			else {
				bool err = false;
				long long it_idx = _stack.back()->GetItemIndexFromIlistIndex(idx, err);
				if (err) { return "#2"; }
				return _stack.back()->GetUserTypeList(it_idx)->GetName();
			}
		}
		std::string GetData() const { 
			...
			bool err = false;
			long long ut_idx = _stack.back()->GetItemIndexFromIlistIndex(idx, err);

			if (err) {
				return "";
			}
			
			auto x = _stack.back()->GetItemList(ut_idx);
			
			return x.Get();
		}
		bool IsGroup() const  {
			return _stack.back()->IsUserTypeList(idx);
		} // object_or_array
		...
		bool Enter() {
			...
			_stack.push_back(_stack.back()->GetUserTypeList(ut_idx));
		...
		long long Length() const  {
			return _stack.back()->GetIListSize();
		}
	...
	public:
		std::string SetKey(const std::string& val) {
			if (IsGroup()) {
				bool err = false;
				long long ut_idx = reader->_stack.back()->GetUserTypeIndexFromIlistIndex(idx, err);
				if (err) { return "#1"; }
				reader->_stack.back()->GetUserTypeList(ut_idx)->SetName(val);
			}
			else {
				bool err = false;
				long long it_idx = reader->_stack.back()->GetItemIndexFromIlistIndex(idx, err);
				if (err) { return "#2"; }
				reader->_stack.back()->GetUserTypeList(it_idx)->SetName(val);
			}
			return "";
		}
		std::string SetData(const std::string& val) {
			...
			bool err = false;
			long long ut_idx = reader->_stack.back()->GetItemIndexFromIlistIndex(idx, err);

			if (err) {
				return "#3";
			}

			auto x = reader->_stack.back()->GetItemList(ut_idx);
		...
		// object_or_array
		bool IsGroup() const {
			return reader->_stack.back()->IsUserTypeList(idx);
		} 
		...
		long long Length() const {
			return reader->_stack.back()->GetIListSize();
		}
	};
